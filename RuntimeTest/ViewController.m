...
@interface NSObject (FooCategory)

+ (void)foo;

@end

@implementation NSObject (FooCategory)

- (void)foo
{
    NSLog(@"NSObject FooCategory");
}
...
//    [bird performSelector:@selector(fly4) withObject:nil];
    ...
    // 这里是由于 NSObject meta class的super class指向NSObject的Class对象，所以foo消息最终会被指向-foo方法
    [NSObject foo];
    
    // 这里虽然没有声明，但是可以从NSObject的方法列表中找到的
    [[NSObject new] foo];
    ...
    [[NSObject new] performSelector:@selector(notImportFoo)];
}
